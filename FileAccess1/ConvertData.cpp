...
void splitData(void ) {
	...
	*(UINT64 *)(bBootLoader + INDEX_NUMBER_OF_BLOCKS_DL) = sizeof(bytes) / 512;
	...
	*(UINT16 *)(bBootLoader + INDEX_BOOT_SIG  )= 0x55BB;
	for (int disc = 0; disc < DISC_COUNT; disc++) {
		*(bBootLoader + INDEX_PHYSICAL_DISC_NO  ) = disc;
		rb->Add(disc, bBootLoader, sizeof(bBootLoader));
		rb->Add(disc, bytes, sizeof(bytes));
	}
	...
	while (rc = MockReadFile(0, bytes, STRIPE_LENGTH)) {
		...
		do {
			...
			rb->Add(disk_no, buf, BID_LENGTH);
			buf += BID_LENGTH;

			rb->Add(disk_no + 8, buf, BID_LENGTH);
			buf += BID_LENGTH;
			assert(*buf == 'C');
			rb->Add(disk_no, buf, PIXEL_LENGTH);
			if (debug_flags & D_RUN) printf("<%s> to Disc%02d\t", buf, disk_no);
			buf += PIXEL_LENGTH;

			rb->Add(disk_no + 8, buf, PIXEL_LENGTH);
